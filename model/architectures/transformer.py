...
class EncoderDecoderTransformer(nn.Module, PyTorchModelHubMixin):
    ...
    def _instantiate_encoder(self, embed_dim, num_heads, ff_dim, num_encoder_layers):
        return EncoderBlock(embed_dim, num_heads, ff_dim, num_encoder_layers)

    def _instantiate_decoder(self, embed_dim, num_heads, ff_dim, num_decoder_layers):
        return DecoderBlock(embed_dim, num_heads, ff_dim, num_decoder_layers)
    ...
    def forward(
        ...
        src = src * math.sqrt(src.size(-1)) + self.positional_encoding[
            :, : src.size(1)
        ].to(src.device)
        tgt = tgt * math.sqrt(src.size(-1)) + self.positional_encoding[
            :, : tgt.size(1)
        ].to(tgt.device)
