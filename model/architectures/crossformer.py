...
class EncoderDecoderCrossFormer(EncoderDecoderTransformer):
    ...
    def __init__(
        ...
        self.decoder_cross_attention_encoder = nn.ModuleList([
            nn.MultiheadAttention(
                ...
                num_heads=num_heads,
                batch_first=True,
                dropout=self.encoder[0].time_attention.dropout  # same dropout throughout network
            )
    ...
    def forward(
        ...
        assert src_seq_len == tgt_seq_len, "Source and Target sequence have different lenghts! Check padding."

        # Create time-wise causal mask for the target sequence
        if tgt_tgt_time_mask is None:
            tgt_tgt_time_mask = self._create_causal_mask(
                tgt_seq_len
            ).to(tgt.device)
        ...
        decoder_output = tgt
        for i, (decoder_layer, cross_attention_to_encoder) in enumerate(zip(
            self.decoder, self.decoder_cross_attention_encoder
        )):
            ...
            decoder_output, _ = cross_attention_to_encoder(
                decoder_for_cross_attention, encoder_output, encoder_output,
